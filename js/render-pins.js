  ...
  var renderPins = function (dataArr, targetList, templateId, templateClass) {
    ...
    for (i = 0; i < dataArr.length; i++) {
      if (dataArr[i].offer) {
        ...
        var pinPositionX = (dataArr[i].location.x <= size.PIN_WIDTH) ? dataArr[i].location.x : dataArr[i].location.x - size.PIN_WIDTH;
        var pinPositionY = (dataArr[i].location.y <= size.PIN_HEIGHT) ? dataArr[i].location.y - size.PIN_HEIGHT / 2 : dataArr[i].location.y;
